...
export function TimeConverter() {
  ...
  const convert = () => {
    ...
    const result = (inputValue * timeUnits[fromUnit].factor) / timeUnits[toUnit].factor
      ...
      <CardContent className="p-0">
        <div className="grid gap-6 p-6">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            <p className="mt-1">
              1 {fromUnit} = {(timeUnits[fromUnit].factor / timeUnits[toUnit].factor).toFixed(6).replace(/\.?0+$/, "")}{" "}
              {toUnit}
            </p>
          </div>
        </div>
